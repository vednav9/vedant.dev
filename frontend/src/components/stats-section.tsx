...
import { GITHUB_FALLBACK, LEETCODE_FALLBACK } from "@/hooks/use-stats";
...
export function StatsSection() {
  ...
  const gh = github.data ?? GITHUB_FALLBACK;
  const lc = leetcode.data ?? LEETCODE_FALLBACK;
  const totalMax = 500;
  const showReadmeStats = process.env.NEXT_PUBLIC_GITHUB_STATS_CARDS === "true";
  const statsTheme = mounted && theme === "dark" ? "dark" : "light";
  ...
  return (
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10" ref={ref}>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-6">
              {[
                { label: "Repositories", value: gh.repos, suffix: "+", icon: Code2, color: "text-violet-400" },
                { label: "Followers", value: gh.followers, suffix: "", icon: TrendingUp, color: "text-blue-400" },
                { label: "Stars", value: gh.stars, suffix: "", icon: Star, color: "text-amber-400" },
              ].map((stat) => (
                <div key={stat.label} className="text-center p-2 sm:p-4 rounded-xl bg-secondary/50 border border-border/30 hover:border-primary/30 transition-colors">
            ...
            <div className="rounded-xl overflow-hidden border border-border/30 bg-muted/30 dark:bg-[#0d1117] p-3">
            ...
            <div className="mt-4 rounded-xl overflow-hidden border border-border/30">
              <img
                src={`https://github-readme-streak-stats.herokuapp.com/?user=${gh.username}&theme=${statsTheme}&background=0d111700&border=30363d&stroke=30363d&ring=7c3aed&fire=3b82f6&currStreakLabel=7c3aed&hide_border=false`}
            ...
            <div className="mt-4 rounded-xl overflow-hidden border border-border/30">
              <img
                src={`https://github-readme-stats-sigma-five.vercel.app/api?username=${gh.username}&show_icons=true&theme=${statsTheme === "dark" ? "react" : "default"}&rank_icon=github&border_radius=10&bg_color=${statsTheme === "dark" ? "0d1117" : "ffffff"}&border_color=${statsTheme === "dark" ? "30363d" : "e2e8f0"}&title_color=7c3aed&icon_color=3b82f6&text_color=${statsTheme === "dark" ? "8b949e" : "475569"}`}
                alt="GitHub Stats"
                className="w-full"
                loading="lazy"
                onError={(e) => {
          ...
          >
            ...
            <div className="flex items-center gap-6 mb-8">
              ...
              <div className="flex-1 space-y-1.5">
                <div className="text-xs text-muted-foreground">
                  Global Rank: <span className="text-foreground font-mono">#{lc.rank.toLocaleString("en-US")}</span>
                </div>
                <div className="text-xs text-muted-foreground">
            ...
            <div className="mt-6 rounded-xl overflow-hidden border border-border/30">
              <img
                src={`https://leetcard.jacoblin.cool/${lc.username}?theme=${statsTheme}&font=Geist+Mono&ext=heatmap`}
        ...
        {/* GitHub stats cards row (optional external service) */}
        {showReadmeStats && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={inView ? { opacity: 1, y: 0 } : {} }
          ...
          >
            <div className="rounded-xl overflow-hidden border border-border/30">
              <img
                src={`https://github-readme-stats.vercel.app/api?username=${gh.username}&show_icons=true&theme=${statsTheme}&bg_color=0d111700&title_color=7c3aed&icon_color=3b82f6&text_color=8b949e&border_color=30363d`}
                alt="GitHub stats"
                className="w-full"
                loading="lazy"
                onError={(e) => {
            ...
            <div className="rounded-xl overflow-hidden border border-border/30">
              <img
                src={`https://github-readme-stats.vercel.app/api/top-langs/?username=${gh.username}&layout=compact&theme=${statsTheme}&bg_color=0d111700&title_color=7c3aed&text_color=8b949e&border_color=30363d`}
                alt="Top languages"
                className="w-full"
                loading="lazy"
                onError={(e) => {
