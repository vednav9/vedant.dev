          ...
          <div className="marquee-mask overflow-hidden">
            <div className="flex gap-2 w-max animate-marquee hover:[animation-play-state:paused]">
              {[...alsoFamiliar, ...alsoFamiliar].map((tag, i) => (
                <span
                  key={`${tag}-${i}`}
                  className="shrink-0 px-3 py-1 text-xs font-mono rounded-lg bg-secondary/70 text-muted-foreground border border-border/40 hover:border-primary/30 hover:text-foreground transition-colors cursor-default"
                >
                  {tag}
                </span>
              ))}
